...
/**
 * Drager Evita 协议分帧：SOH 或 ESC 起始，CR 终止。
 * payload 保留完整协议帧，包括起始字节和结尾 CR。
 */
public final class DragerEvitaFrameDecoder implements FrameDecoder {
    @Override
    ...
    public Optional<byte[]> softTrim(byte[] buffer) {
        ...
        split = findLast(buffer, Consts.ESC);
        if (split >= 0) {
            return Optional.of(Arrays.copyOfRange(buffer, split, buffer.length));
        }
    ...
    private static int findStart(byte[] data, int from) {
        for (int i = Math.max(0, from); i < data.length; i++) {
            if (data[i] == Consts.SOH || data[i] == Consts.ESC) {
                return i;
            }
        }
        return -1;
